...
int CF_wait_data (void)
{
	...
	do {
		if (!Timer1) return 0;		/* Abort when timeout occured */
...
static
int MM_wait_ready (void)
{
	Timer2 = 50;	/* Wait for ready in timeout of 500ms */
	MM_rcvr_spi();
	do
		if (MM_rcvr_spi() == 0xFF) return 1;
	while (Timer2);
...
static
int MM_select (void)	/* 1:Successful, 0:Timeout */
{
	CS_LOW();
	if (!MM_wait_ready()) {
		MM_deselect();
...
static
BYTE MM_send_cmd (	/* Returns R1 resp (bit7==1:Send failed) */
...
)
{
	...
	MM_deselect();
	if (!MM_select()) return 0xFF;

	/* Send command packet */
	MM_xmit_spi(0x40 | cmd);			/* Start + Command index */
...
DSTATUS CF_disk_initialize (void)
{
	...
	Timer1 = 250;								/* Wait for card goes ready */
	while (Timer1 && CF_read_ata(REG_STATUS) & BUSY) ;
	...
	for (Timer1 = 2; Timer1; );					/* 20ms */
	Timer1 = 250;								/* Wait for card goes ready */
	while (Timer1 && (CF_read_ata(REG_STATUS) & (DRDY|BUSY)) != DRDY) ;
	if (!Timer1) return Stat[0];
